...
export default function DashEventRegistrations() {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        if (res.ok) {
          setRegisterEvents(data.registrations);
  ...
  //   const startIndex = registerEvents.length;
  //   try {
  ...
  //     const data = await res.json();
  //     if (res.ok) {
  //       setRegisterEvents((prev) => [...prev, ...data.events]);
  //       if (data.events.length < 9) {
  //         setShowMore(false);
  //       }
  //     }
  ...
  return (
    <div className="table-auto overflow-x-scroll md:mx-auto p-3 scrollbar scrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500">
      {currentUser.isAdmin && registerEvents.length > 0 ? (
        <>
